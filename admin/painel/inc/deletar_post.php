...
if(isset($_GET['ac'])){
	if($_GET['ac'] == 'deletar'){
		$post = new Post;
		$post->setId((int)$_GET['id']);
		$dados_post = $post->post_Id();
		if(unlink('../../'. $dados_post->post_foto)){
			if($post->deletar()){
			echo '<p class="alert alert-success"><strong>Post</strong> foi removido com sucesso !</p>';
			} else {
				echo '<p class="alert alert-danger"><strong>Post</strong> não pode ser removido !</p>';
			}
		}else {
			...
			<table class="table table-bordered" border="1" style="width: 700px">
				...
				foreach ($data as  $value) {
					...
					<tr>
						...
						<td style="width: 100px"><a href="?p=deletar_post&ac=deletar&id=<?php  echo $value->post_id ?>"><button class="btn btn-danger">Deletar</button></a></td>
					</tr>
