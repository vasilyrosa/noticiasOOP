...
class Post extends Abstrata  {
	...
	public function listarUltimas() {
		...
		$this->setParametros(" INNER JOIN categoria INNER JOIN administrador ON
												...
												ORDER BY post_data DESC LIMIT 4
												");
	...
	public function listarNoticiaDestaque() {
		parent::$tabela = "post";
		parent::$existeParametros = true;
		$this->setParametros(" INNER JOIN  categoria INNER JOIN  administrador ON
												post.post_categoria = categoria.categoria_id AND
												post.post_autor = administrador.adm_id
												WHERE categoria_posicao_destaque = ".$this->getId()."
												ORDER BY  post_data DESC LIMIT
												".$this->getLimite());
		 return parent::listar();
	}
